    ...
    def get_iter(self, dataset, domain='soccer'):
        ...
        for j in range(0, len(data), self.batch_size):
            ...
            if len(batch_data) < self.batch_size:
                q, q_m, i_g, y, y_m, i_e, t_y, c_e, i_q, o_r, c_o, l_kg = self._load_batches(batch_data, dat=domain)
            else:
                q, q_m, i_g, y, y_m, i_e, t_y, c_e, i_q, o_r, c_o, l_kg = self._load_batches(batch_data, dat=domain)

            yield q, q_m, i_g, y, y_m, i_e, t_y, c_e, i_q, o_r, c_o, l_kg
    ...
    def _load_batches(self, batch_data, dat='soccer'):
        ...
        for b, data in enumerate(batch_data):
            ...
            for k, w in enumerate(query):  # Get word embeddings
                q[b, k] = self.get_w2i(w, self.src_stoi)
                ...
                for k, v in data['h'].items():
                    try:
                        ...
                        try:
                            ...
                            continue
        if self.gpu:
            q, q_m, y, y_m, i_e, i_g = q.cuda(), q_m.cuda(), y.cuda(), y_m.cuda(), i_e.cuda(), i_g.cuda()

        return q.long(), q_m, i_g, y.long(), y_m, i_e.long(), true_answer, correct_ent, inp_query, relations, correct_obj, local_kg


if __name__ == '__main__':
    batcher = InCarBatcher()
    batches = batcher.get_iter('val', domain='incar')
    for e, b in enumerate(batches):
        # print (b)
        q, q_m, i_g, y, y_m, i_e, t_e, c_e, i_q, o_r, c_o, l_kg = b
        print (l_kg)
